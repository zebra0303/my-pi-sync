...
2. Offer to apply the fix manually — call `update_self_healing_fix` with action `APPLY` if user agrees
3. Record `last_cipe_url`, enter wait mode

### fix_apply_ready

- Call `update_self_healing_fix` with action `APPLY`
...
1. **Detect package manager:** `pnpm-lock.yaml` → `pnpm nx`, `yarn.lock` → `yarn nx`, otherwise `pnpm nx`
2. **Run verifiable tasks in parallel** — run focused local verification commands for each task, using parallel tool calls only when safe
3. **If all pass** → call `update_self_healing_fix` with action `APPLY`, enter wait mode
4. **If any fail** → Apply Locally + Enhance Flow (see below)

### fix_needs_review

Call `ci_information` with HEAVY_FIELDS, then analyze fix content (`suggestedFixDescription`, `suggestedFixSummary`, `taskFailureSummaries`):
...
Call `ci_information` with HEAVY_FIELDS for `taskFailureSummaries`. Run `ci-state-update.mjs gate --gate-type local-fix` — if not allowed, print message and exit. Otherwise attempt local fix (counter already incremented by gate). If successful → commit, push, enter wait mode. If not → exit with failure.

### environment_issue

1. Run `ci-state-update.mjs gate --gate-type env-rerun`. If not allowed, print message and exit.
2. Call `update_self_healing_fix` with action `RERUN_ENVIRONMENT_STATE`
3. Enter wait mode with `last_cipe_url` set

### self_healing_throttled

Call `ci_information` with HEAVY_FIELDS for `selfHealingSkipMessage`.
...
Call `update_self_healing_fix` with action `APPLY`. New CI Attempt spawns automatically. No local git ops.
...
2. Call `update_self_healing_fix` with action `REJECT`
