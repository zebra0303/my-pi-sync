...
You are the orchestrator for monitoring Nx Cloud CI pipeline executions and handling self-healing fixes in pi. Use the available Nx Cloud MCP tools when present, run deterministic decision scripts, and take action based on the results.

## Initial Context

Collect this context with pi tools before starting:
...
**Important:** If the user provides specific instructions in the chat or `/skill:monitor-ci ...` arguments, respect them over default behaviors described below.
...
Parse any overrides from the user's message or skill command arguments and merge with defaults.
...
1. **This skill (orchestrator)**: calls Nx Cloud MCP tools, runs scripts, prints status, does local coding work
2. **Nx Cloud MCP calls**: call `ci_information` or `update_self_healing_fix` using the available MCP/custom tool surface. If those tools are not available in the pi session, stop and tell the user Nx Cloud MCP is required for self-healing CI monitoring.
...
2. Tell the user that Nx Cloud MCP tools are required to continue self-healing monitoring in pi
3. Do not continue polling on the main agent — it wastes context tokens and bypasses self-healing
...
#### 2a. Fetch status via Nx Cloud MCP
...
Call the `ci_information` MCP/custom tool with the determined `select` fields for the current branch. Wait for the result before proceeding. If the tool is unavailable, stop and report that Nx Cloud MCP is required.

#### 2b. Run decision script

```bash
node <skill_dir>/scripts/ci-poll-decide.mjs '<ci_information_result_json>' <poll_count> <verbosity> \
  [--wait-mode] \
...
- `prev_cipe_status = ci_information_result.cipeStatus`
- `prev_sh_status = ci_information_result.selfHealingStatus`
- `prev_verification_status = ci_information_result.verificationStatus`
- `prev_failure_classification = ci_information_result.failureClassification`
- `prev_status = output.action + ":" + (output.code || ci_information_result.cipeStatus)`
- `poll_count++`
