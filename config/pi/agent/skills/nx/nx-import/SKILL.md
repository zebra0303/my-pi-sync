...
- In pi, always prefix Nx with the detected workspace package manager (`pnpm nx`, `yarn nx`, `npm exec nx --`, etc.). Examples use bare `nx`; translate them before executing.
...
- **Subdirectory import**: Plugins NOT auto-detected. Manually add with `pnpm nx add @nx/PLUGIN` (or the detected package-manager equivalent). Check `include`/`exclude` patterns — defaults won't match alternate directories (e.g. `apps-beta/`).
- Run `pnpm nx reset` (or equivalent) after any plugin config changes.
...
3. Then `pnpm nx add @nx/eslint` (or equivalent) to register the plugin in `nx.json`
...
1. Run `pnpm nx add @nx/jest` (or equivalent) — registers `@nx/jest/plugin` in `nx.json` and updates `namedInputs`
